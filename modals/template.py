...
from PyQt5.QtGui import QPixmap, QPen, QColor, QMouseEvent, QKeyEvent
from PyQt5.QtWidgets import QGraphicsScene, QFileDialog, QGraphicsPixmapItem, QGraphicsRectItem, \
    QGraphicsItem, QGraphicsView, QGraphicsSceneMouseEvent, QMessageBox, QMainWindow
...
class GraphicsRectItem(QGraphicsRectItem):
    ...
    def mouseDoubleClickEvent(self, event: QGraphicsSceneMouseEvent):
        print(self.rect().topLeft(), self.rect().y(), self.rect().x())
...
class GraphicsView(QGraphicsView):
    ...
    def keyPressEvent(self, event: QKeyEvent):
        ...
        if event.key() == Qt.Key_Control:
            ...
            #         self.start_coords = item.pos().x(), item.pos().y()
            #         break
    ...
    def mouseMoveEvent(self, event: QMouseEvent):
        if self.start_coords and self.parent.is_edit_mode() and event.buttons() & Qt.LeftButton:
            ...
            w, h = int(self.end_coords[0] - self.start_coords[0]), int(self.end_coords[1] - self.start_coords[1])
            self.current_item.setRect(QRectF(int(self.start_coords[0]), int(self.start_coords[1]), w, h))
        else:
    ...
    def mousePressEvent(self, event: QMouseEvent):
        if not self.start_coords and self.parent.is_edit_mode() and event.buttons() & Qt.LeftButton:
            ...
            rect_item = GraphicsRectItem(QRectF(int(self.start_coords[0]), int(self.start_coords[1]), 1, 1))
